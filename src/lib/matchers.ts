...
import { GoDefaultError } from "../models/errors/go-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | GoDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | GoDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new GoDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new GoDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
